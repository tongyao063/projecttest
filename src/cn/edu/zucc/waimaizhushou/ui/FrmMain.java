...
public class FrmMain extends JFrame implements ActionListener {
	...
	private void reloadPlanTable(){//这是测试数据，需要用实际数替换
		try {
			allPlan=WaiMaiUtil.planManager.loadAll();
		...
		for(int i=0;i<allPlan.size();i++){
			for(int j=0;j<BeanPlan.tableTitles.length;j++)
				tblPlanData[i][0]=allPlan.get(i).getPlan_order();
				tblPlanData[i][1]=allPlan.get(i).getPlan_name();
				tblPlanData[i][2]=allPlan.get(i).getStep_count();
				tblPlanData[i][3]=allPlan.get(i).getFinished_step_count();
		}
	...
	private void reloadPlanStepTabel(int planIdx){
		...
		try {
			planSteps=WaiMaiUtil.stepManager.loadSteps(curPlan);
		} catch (BaseException e) {
			JOptionPane.showMessageDialog(null, e.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		for(int i=0;i<planSteps.size();i++){
			for(int j=0;j<BeanStep.tblStepTitle.length;j++)
				tblStepData[i][0]=planSteps.get(i).getStep_order();
				tblStepData[i][1]=planSteps.get(i).getStep_name();
				tblStepData[i][2]=planSteps.get(i).getPlan_begin_time();
				tblStepData[i][3]=planSteps.get(i).getPlan_end_time();
				tblStepData[i][4]=planSteps.get(i).getReal_begin_time();
				tblStepData[i][5]=planSteps.get(i).getReal_end_time();
		}
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.menuItem_AddPlan){
			...
			try {
				WaiMaiUtil.planManager.deletePlan(this.curPlan);
			...
			try {
				WaiMaiUtil.stepManager.deleteStep(this.planSteps.get(i));
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				WaiMaiUtil.stepManager.startStep(this.planSteps.get(i));
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				WaiMaiUtil.stepManager.finishStep(this.planSteps.get(i));
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				WaiMaiUtil.stepManager.moveUp(this.planSteps.get(i));
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				WaiMaiUtil.stepManager.moveDown(this.planSteps.get(i));
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
